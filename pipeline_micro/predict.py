...
from plots import plot_prediction_results


def predict(data_dir=None, test_labels_file=None, artifacts_dir=None):
    """
    ...
    test_labels_file = test_labels_file or config.TEST_LABELS_FILE
    artifacts_dir = Path(artifacts_dir or config.ARTIFACTS_DIR)

    write_progress(artifacts_dir, phase="starting", message="Lade Test-Labels...")
    # FEATURE_SET und feat_cols aus erstem Modell laden (müssen übereinstimmen)
    _ld = joblib.load(artifacts_dir / f"model_{MODELS[0]}.joblib")
    FEATURE_SET, feat_cols = _ld[2], _ld[1]

    paths, ids = load_labels(test_labels_file, data_dir)
    if not paths:
    ...
    X = result[feat_cols]
    classes = sorted(result["driver_id"].unique().tolist())
    ...
    for i, mdl in enumerate(MODELS):
        ...
        # Klassen-Reihenfolge auf unsere classes mappen
        ...
        agg = pdf.groupby("rec", sort=False).agg({**{c: "mean" for c in classes}, "y_true": "first"})
        recs = [{"recording": r, "soll": agg.loc[r, "y_true"], "ist": classes[np.argmax(agg.loc[r, classes].values)], "korrekt": classes[np.argmax(agg.loc[r, classes].values)] == agg.loc[r, "y_true"]} for r in agg.index]
        out = artifacts_dir / f"test_ergebnis_{mdl}.csv"
        pd.DataFrame(recs).to_csv(out, index=False)
        plot_prediction_results(recs, mdl, artifacts_dir)
