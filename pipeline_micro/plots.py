...
def plot_prediction_results(
    ...
    """
    Erstellt Grafik: Welche Vorhersagen waren richtig/falsch.
    Speichert in out_dir/plots/prediction/.
    Args:
        recs: [{"recording": str, "soll": str, "ist": str, "korrekt": bool}, ...]
    ...
    try:
        if not recs:
            return None
        labels = [r["recording"].replace(".csv", "")[:45] for r in recs]
        correct = [1 if r["korrekt"] else 0 for r in recs]
        colors = ["#2ecc71" if c else "#e74c3c" for c in correct]
        fig, ax = plt.subplots(figsize=(10, max(4, len(recs) * 0.45)))
        y_pos = np.arange(len(labels))
        ax.barh(y_pos, [1] * len(labels), color=colors, alpha=0.85, height=0.7)
        ax.set_yticks(y_pos)
        ...
        ax.set_xlabel("")
        n_ok = sum(correct)
        ax.set_title(f"Vorhersageergebnis – {model_name}  |  Korrekt: {n_ok}/{len(recs)}  (grün=richtig, rot=falsch)")
        plt.tight_layout()
        plots_dir = _ensure_dir(out_dir / "plots" / "prediction")
        out_path = plots_dir / f"prediction_{model_name}.png"
        fig.savefig(out_path, dpi=100, bbox_inches="tight")
        plt.close(fig)
        return out_path
    except Exception:
