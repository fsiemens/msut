...
def write_labels_file(file_paths, output_path):
    """
    Schreibt Label-Datei (CSV mit File,Label) aus Liste von Dateipfaden.
    Extrahiert Fahrer aus Dateinamen (florian, matthias, fabian). Schreibt absolute Pfade.
    ...
    def label_from_filename(name):
        n = str(name).lower().replace(".csv", "")
        for driver in ["florian", "matthias", "fabian"]:
            if driver in n:
                return driver
        parts = n.split("_")
    ...
    for fp in file_paths:
        p = Path(fp)
        name = p.name if p.is_absolute() else str(fp)
        # Absoluter Pfad für maximale Robustheit (data.load_labels unterstützt beides)
        lines.append(f"{p.resolve()},{label_from_filename(name)}")
