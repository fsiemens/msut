...
def extract_features(paths, ids, feature_set=FEATURE_SET, on_extraction_start=None):
    """
    ...
    if feature_set in ("featuretools", "both"):
        ...
        result_ft = win_df.merge(fm.reset_index(), on="window_id", how="left").drop(columns=["window_id"])
    ...
    if feature_set in ("tsfresh", "both"):
        ...
        if ts_rows:
            ...
            result_ts = win_df[["driver_id", "recording"]].copy()
            for c in ts_feat.columns: result_ts[c] = ts_feat[c].values

    # Ergebnis aus beiden Quellen zusammensetzen (bei "both")
    result = result_ft.copy() if feature_set == "both" else (result_ts if feature_set == "tsfresh" else result_ft)
    if feature_set == "both":
