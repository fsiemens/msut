...
Erzeugt eine Label-Datei aus einer Liste von Dateipfaden und zugehörigen Labels. Die Labels stammen vom Aufrufer (z.B. aus einer bestehenden Label-Datei oder Nutzereingabe im GUI).
...
| `labels`      | `list[str]`            | Liste der Fahrer-Labels (parallel zu file_paths)  |
| `output_path` | `str` oder `Path`      | Ziel-Label-Datei (z.B. `labels.lbl`)              |

**Beispiel:**
```python
# Labels aus bestehender Datei laden und speichern
from data import load_labels
paths, ids = load_labels("labels.lbl", "data")
write_labels_file(paths, ids, "labels.lbl")
```
